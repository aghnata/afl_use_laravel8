...
<body>
    ...
    <h4>Invoice {{$sortedAfleeName}} <br> {{$finalStartDate}} hingga {{$finalEndDate}}</h4>
    ...
    <table id="customers">
      ...
      <tr>
        ...
        <td> {{ number_format($schedule->cost_per_session) }} </td>
        <td> {{ $schedule->sum_session }} </td>
        <td> {{ number_format($schedule->cost_per_session*$schedule->sum_session) }} </td>
        <td>{{ isset($schedule->location()->cost) ? number_format($schedule->location()->cost) : '' }}</td>
        <td> {{ number_format($schedule->total_cost) }} </td>
      </tr>
      @endforeach
